...
//						System.out.println("终端握手类型："+byteType);
						...
						try {
							...
							logger.error("出错日志  记录："+e.getMessage()+"  信息：Socket错误,channelRead" );
						}
						logger.debug("跟踪日志  记录："+" 终端握手类型：" + byteType + " 命令：" + byteOrder+"  信息：" + address );
	...
	public void channelInactive(ChannelHandlerContext ctx) throws Exception {
		...
		logger.debug("跟踪日志  记录：终端中断连接  信息：" +address);
		ctx.close(); 
	}
	@Override
	...
	public void AcceptData(ChannelHandlerContext ctx,String byteType,String byteOrder,String acceptData,byte[] acceptbyte){
		...
		try{
			...
			case "0"://网页发来消息，直播结束，删除直播列表中的该直播id
				...
				logger.info("正常日志  记录：前台直播结束命令  信息：直播" +streamid);
				break;
			case "1"://网页发来消息，直播开始
				isEndSocket = false;
...
//	        		terstreamlist.add(new TerStream(strid,imeis[i],true));//测试
				...
				ReturnData = "".getBytes(UTF8);
				logger.info("正常日志  记录：前台直播开始命令  信息：直播" +strid);
				break;
			case "2"://网页请求终端直播状态
				...
				logger.info("正常日志  记录：前台请求终端直播状态  信息：直播" +getstreamid);
				break;
			case "12"://流媒体心跳包
				imei = acceptData;
//				//测试
...
//				//测试-----end
				...
				logger.info("正常日志   记录：流媒体心跳包    信息："+ imei+":"+txt);
				break;
			case "14"://终端发送流媒体播放成功指令
				...
				logger.info("正常日志   记录：流媒体播放成功    信息："+ imei);
				break;
			case "15"://终端发送终止流媒体播放成功指令
				...
				logger.info("正常日志   记录：流媒体终止播放成功    信息："+ imei);
				break;
				}
			SendToByte(ctx,byteType,byteOrder,ReturnData);
		} catch (UnsupportedEncodingException e) {
			logger.error("出错日志  记录："+e.getMessage()+"  信息：Socket错误,AcceptData" );
		}
	}
	private void SendToByte(ChannelHandlerContext ctx,String byteType,String byteOrder, byte[] SendData){
		if (SendData != null && SendData.length != 0){
			...
			try {
				...
				logger.error("出错日志  记录："+ex.getMessage()+"  信息：Socket错误,SendToByte  byteType:" + byteType + ",byteOrder:" + byteOrder);
			}
		}else{
			try {
				int buffersize = 7;
